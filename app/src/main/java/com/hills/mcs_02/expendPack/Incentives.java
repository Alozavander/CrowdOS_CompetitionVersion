...
public interface Incentives {
     ...
     */
    public void incentive_get(Task task, BeanUserAccount account);
    public void incentive_pay(Task task, BeanUserAccount account);

    /*
     * @param account 账户类，包含用户名等相关信息
     * @param key 私钥，用于检验解密服务器端传回的验证信息
     */
    public String incentive_verify(BeanUserAccount account, PublicKey key);
}
